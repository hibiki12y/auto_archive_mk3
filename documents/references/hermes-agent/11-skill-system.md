...
| 텔레메트리 | bump_use/view/patch JSON | trait usage stats로 등가 |
| Hub 설치 | GitHub/URL/optional | **SKIP** — 단일 카탈로그 |

## 7. Adoption Notes

**PORT (개념) — M2 일부.** 4개 사상 차용: (1) **USER 메시지 주입** — system 갱신 금지로 prompt cache invariant(`05-...`) 보호, (2) **선언적 config + 정형 prefix** — `metadata.hermes.config ↔ skills.config.<key>` 패턴 매핑 (M2/M3), (3) **라이프사이클 ACTIVE/STALE/ARCHIVED** — `archive`만 사용, `delete`는 사용자 명시 명령에만, (4) **Pinned write-block** — 사용자 pin은 curator 미수정 (RELEASE_v0.12.0 §97).
