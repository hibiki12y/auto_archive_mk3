...
기본값은 `AUTO_ARCHIVE_CODEX_AUTH_SOURCE=auto`이다.
...
- `AUTO_ARCHIVE_CODEX_AUTH_SOURCE=codex-cli`: local CLI auth를 필수로 요구하고
  없으면 fail-closed.
- `AUTO_ARCHIVE_CODEX_AUTH_SOURCE=api-key`: API 키를 필수로 요구하고 local CLI
  auth 검사를 건너뜀.
...
- Docker 서비스는 `AUTO_ARCHIVE_CODEX_CLI_HOME_MODE=isolated-auth`를 기본으로
  사용하여 `auth.json`만 child-process Codex home에 연결하고 host
  `~/.codex/config.toml` / `~/.codex/.env` 설정은 격리함
