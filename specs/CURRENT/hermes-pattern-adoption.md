...
source_paths:
  ...
  - CODE_STANDARDS.md
scope: Hermes Agent v0.12.0 패턴 채택 추적 — 16개 서브시스템에 대한 PORT/PORT-PARTIAL/SKIP 결정과 각 결정이 매핑되는 Phase B M-item 진행 상태. 본 문서는 채택 결정의 영구 기록이며 M-item이 land될 때마다 갱신된다.
supersedes:
---
...
| --- | --- | --- |
| 06 Provider adapters | auto_archive_mk3는 codex-sdk + claude-agent-sdk만 사용. native adapter 작성 불필요 — SDK가 추상화 제공. | 신규 provider(예: Gemini) 직접 통합 필요 시 |
| 07 Trajectory compression | RL training은 본 프로젝트 범위 밖. | RL/DPO/SFT 데이터셋 생성이 로드맵에 진입 시 |
| 08 Batch runner | 동상. | 동상. |
| 09 RL environments | 동상. atroposlib 의존도 추가 부담. | 동상. |
| 15 Trajectory hooks | RL 범위 밖, 분량도 미미(57 LOC). | RL 활성화 시 |
| 16 Credential pool | 우리는 단일 provider bootstrap, multi-key rotation 불필요. | multi-tenant SaaS 모드 진입 시(현재 로드맵 없음) |
