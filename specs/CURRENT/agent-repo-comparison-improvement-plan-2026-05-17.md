...
source_paths:
  - PROJECT.md
  - README.md
  - specs/ARCHIVE/architecture-hexagonal-microkernel.md
  ...
  - specs/ARCHIVE/openclaw-gap-implementation.md
  - specs/ARCHIVE/orchestrator-subagent-skill-pattern.md
...
## 1.0. 2026-05-21 범위 정제

사용자 결정에 따라 이 계획은 Auto Archive를 **자동 연구 워크플로/증거 거버넌스 플랫폼**으로 정리한다. 모델 자동 학습, RL/SFT-DPO, trajectory compression, batch-runner 학습 데이터화, provider zoo는 현시점 개량 목표가 아니다. 아래 G8/G11의 "self-improvement/eval"은 모델 가중치 학습이 아니라 constraint report, claim/evidence coverage, operator handoff, reusable skill candidate 같은 **reviewable workflow artifact** 개선 루프로만 해석한다. 현재 readiness posture와 아카이브된 2026-05-16 gate는 `specs/CURRENT/research-platform-readiness-and-scope-2026-05-21.md`를 따른다.
...
3. 다중 입력 채널, 모바일/음성/캔버스, 모델 자동 학습/RL처럼 당분간 상관없는 범위는 무엇을 제외할 것인가?
...
| RL trajectory/batch-runner 학습 데이터화 | Hermes Agent, Super Hermes 일부 실험 흐름 | 현재 연구 아카이브 제품 목표와 직접 연결되지 않는다. constraint report 수준의 워크플로 개선 루프만 검토한다. |
| 모델 자동 학습/SFT-DPO/가중치 업데이트 | Hermes RL environments, atroposlib 계열 | 현시점 Auto Archive는 training framework가 아니다. 데이터셋 생성, reward 설계, 학습 실행, 모델 배포를 개량 계획에서 제외한다. |
| trajectory compression/hook 기반 prompt rewrite | Hermes trajectory compression/hooks | raw transcript/trajectory를 durable training material로 삼지 않는다. 필요한 개선은 metadata-only constraint report와 promotion gate로 제한한다. |
...
| Skills/traits | TraitModule, methodology origin, usage telemetry, scheduler slice가 있다. | Hermes/Super Hermes식 모델 self-improvement가 아니라, workflow constraint/eval loop가 아직 충분한 실사용 artifact로 닫혀 있지 않다. |
...
| Workflow improvement/eval loop | Trait/curator 표면은 있으나 blind spot, constraint, reusable skill candidate, claim/evidence coverage를 reviewable artifact로 닫는 루프가 약하다. 모델 가중치 학습/RL은 제외한다. | G8, G11 |

## 6. 미진한 부분과 개량 계획

### G0. Readiness source-of-truth 정리와 golden research mission proof

- **문제**: 2026-05-16 full-matrix blocker/WARN checkpoint는 historical 상태가 되었지만,
  product boundary와 retained replay/fresh-live proof 의미가 여러 문서에 섞여 있었다.
- **비교 신호**: Hermes/OpenClaw/Managed Agents 계열은 README에서 즉시 실행 가능한 quickstart와
  session/service loop를 전면에 둔다. Auto Archive는 static scorecard와 retained proof 원칙이 강하므로,
  이제 "blocked row queue"보다 source-of-truth 정렬과 golden mission proof가 우선이다.
- **개량**:
  1. `research-platform-readiness-and-scope-2026-05-21.md`를 현재 readiness 및 scope SSoT로 유지한다.
  2. 2026-05-16 `full-matrix-release-blockers` / `release-readiness-checkpoint`는 `specs/ARCHIVE/`의 역사적 ledger로만 보존한다.
  3. 신규 기능 구현보다 먼저 research plan → evidence/claim ledger → human/approval gate → synthesis/closeout → proof replay를 잇는 golden research mission proof를 유지한다.
  4. 각 artifact는 기존 scorecard CLI로만 판정하고 raw token/prompt/response는 남기지 않는다.
- **우선순위**: P0.
- **완료 조건**: aggregate `live:proof:report`가 active row에서 `operatorApproved:true`,
  `status:"pass"`를 보고하고, golden mission artifact가 retained replay와 fresh-live 여부를 명확히 구분한다.
...
### G8. Skill/Trait workflow-improvement loop의 검증 부족
...
- **비교 신호**: Super Hermes는 prism/constraint transparency를 통해 분석 절차 자체를 artifact화한다.
  Hermes는 skills/procedural memory와 self-improvement loop를 강조하지만, Auto Archive는 이를 모델 학습이 아닌 workflow artifact 개선으로만 축소 적용한다.
- **개량**:
  ...
  3. Trait/skill 후보 promotion은 자동 적용하지 않고 user/operator approval 또는 project-memory promotion gate를 통과해야 하며, 모델 weight update나 RL dataset generation으로 해석하지 않는다.
- **우선순위**: P2.
- **완료 조건**: repeated research mission에서 blind spot/constraint report가 누적되되,
  raw user content, unapproved prompt rewrite, trajectory-derived training material은 durable memory로 승격되지 않는다.
...
| --- | --- | --- | --- | --- |
| Wave 0 | source-of-truth/readiness 정렬과 golden proof | G0, G7 일부 | 현재 worktree 정리와 operator 승인 | current scope/readiness SSoT, archived 2026-05-16 gates, retained active proof PASS, golden research mission proof |
...
| Wave 3 | workflow-improvement와 onboarding 제품화 | G8, G9 | Wave 2 schema 안정화 | `ResearchConstraintReportSnapshot` + `/critique action:record`, closeout constraint-report eval, and `quickstart:doctor` first-run journey landed; remaining: broader promotion gate tests and accumulated real-mission constraint report review; model auto-learning/RL remains deferred |
...
| 모델 자동 학습/RL/trajectory training은 현재 하지 않는다. | deferred/out-of-scope | 사용자가 training platform을 명시적으로 요구하고 데이터/프라이버시/평가/컴퓨트 gate를 승인할 때 |
...
5. Trait/workflow-improvement는 자동 prompt rewrite, 모델 학습, trajectory mining이 아니라 reviewable artifact와 promotion gate로만 들어온다.
