...
| 8 | `specs/ARCHIVE/` | 역사 전용. 결코 현재 진실이 아님 |
...
- 아카이브 파일이 활성 비-아카이브 파일과 충돌하면 비-아카이브 파일이 이긴다
- `specs/ARCHIVE/`가 정본 아카이브 경로이다. 소문자 `specs/archive/` 트리는
  리다이렉트 전용 호환 스캐폴드이며 새 라이브 문서를 받지 않는다
