...
### Plana advisor provider hot-swap (1.5.0 추가)

운영자는 Plana advisor의 *provider*도 dispatch 경계에서 변경할 수 있다 (`/config set persona:plana key:provider value:<codex|claude-agent>`).

- 구현: `src/core/multi-provider-plana-advisor.ts`. 부트스트랩이 양 sub-advisor를 동시에 instantiate한 뒤(`PlanaClaudeRuntimeAdvisor` + `PlanaCodexRuntimeAdvisor`), `MultiProviderPlanaAdvisor`가 둘을 보유. 매 `review()` 호출 시 `RuntimePersonaSettingsProvider.readSettings('plana').provider` 조회 → 부트스트랩 기본값으로 폴백 → 해당 sub-advisor에 위임.
- 활성 조건: 부트스트랩 시점에 *양쪽* advisor 인증 경로(Claude `~/.claude` OAuth/API key, Codex `auth.json`/API key)가 모두 가용해야 다중-advisor 모드 진입. 한 쪽만 가용하면 레거시 단일-advisor 모드 유지.
- 운영자 검증: 1.4.0과 동일한 부트스트랩-availability 검증을 양쪽 persona에 적용 — target provider가 가용이 아니면 reject. store 무결성 보장.
- Plana 침범 invariant 보존: 양쪽 sub-advisor 모두 *single-shot, no tools, no recursion, fail-open*. wrapper는 dispatch 라우팅만 담당하며 advisor 본체의 제약을 단축하지 않는다.
- 진행 중인 advisor 호출에 대한 무영향성: 1회 `review()` 호출은 정확히 1 sub-advisor를 사용. 다음 `review()` 호출이 새 provider를 본다 (per-instance call cap 또한 sub-advisor별로 독립).
- Audit: `MultiProviderPlanaAdvisor` 자체에는 별도 ledger가 없다 — sub-advisor의 audit ledger(claude: `JsonlPlanaClaudeAdvisorAuditLedger`, codex: optional sink)가 자기 호출만 기록하며, ledger 양분이 곧 routing 증거다.
...
8. Plana advisor provider hot-swap (1.5.0): 부트스트랩 시점 양 advisor 인증이 가용하면 `MultiProviderPlanaAdvisor`가 활성화되어 매 `review()` 시 settings provider의 `plana.provider` 키를 조회하여 sub-advisor에 위임한다. 한 `review()` 호출은 정확히 하나의 sub-advisor만 사용 (fan-out 금지). per-instance call cap은 sub-advisor별로 독립이며, fail-open invariant는 sub-advisor가 그대로 보존한다. `/config set persona:plana key:provider`는 target이 부트스트랩-time 가용이 아니면 reject한다.
