...
The parent Opus orchestrator runs the standard templestay lifecycle unchanged.
...
3. No lighter path is sufficient — single Opus + test execution,
   `templestay-deep-think` lenses, `templestay-codex-delegation` Tier 1 with
   bounded refinement, or CRLN consultation.
...
Three axes; one Opus 4.7-max leader per axis; three leaves per axis drawn
from the approved pool. Phase 1 teams are isolated.
...
Leaf gateway: `mcp-servers/codex-gateway/server.py` for GPT-class leaves;
`mcp-servers/gemini-gateway/server.py` for Gemini leaves. All leaf calls
are read-only. The axis-leader role is dispatched as `dt-audit-axis-leader` from `claude/templestay/agents/`.

## Phase sequence (11 phases)

| Phase | Name | Owner | Required input | Required output | Gate |
|---|---|---|---|---|---|
| 0 | Task Freezing | Parent (Opus) | User request + eligibility trigger | `frozen_input_snapshot_id`, hypothesis lock, axis assignments | All actions succeed; fail-closed |
...
| `templestay-deep-think` | Lighter, lens-based, single-Opus. Pick deep-think when one or two lenses change the approach. No auto-escalation from deep-think to this skill. |
| `templestay-codex-delegation` | Code authoring — Codex-routed. This skill is for audit, not authoring. If the audit produces an action recommendation, dispatch the action through `templestay-codex-delegation`, not this skill. |
...
Parent-internal reasoning steps (Phase 1.5 ledger normalization, Phase 5
lesson extraction) may delegate synthesis to `codex_prompt` to conserve
Claude quota. The contracted axis-leaf topology, Phase 3.25 verifier vendor
identity (must be OpenAI-family), and Phase 4 stress tester vendor identity
(must be Google-family) are unchanged — those slots carry hard vendor-
diversity contracts that override cost concerns.
